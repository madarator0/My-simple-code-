...
pair<int, int> checkD(int x, int y, int ch, int player) {
    int nx = 1, ny = 1;
    if (ch == DL || ch == UL) {
        ny *= -1;
    }
    ...
    if (checkBoard(x, y) && board[x][y].team != player) {
        x += nx;
        y += ny;

        while (checkBoard(x, y) && board[x][y].team != player) {
            if ((board[x][y].team * -1) == player) {
                break;
            }
            x += nx;
            y += ny;
...
pair<int, int> checkP(int x, int y, int ch, int player) {
    int nx = 0, ny = 0;
    if (ch == DR || ch == UR) {
        ny = 0;
        if (ch == DR) {
...
vector<pair<int, int>> allMoves_P(int x, int y, int player) {
    ...
    if (player == W) {
        if (x == 1) {
            ...
            if (checkBoard(x+1, y+1) && board[x+1][y+1].team == (player * -1)) {
...
vector<pair<int, int>> allMoves_K(int x, int y, int player) {
    ...
    for (int i = -1; i <= 1; ++i) {
        for (int j = -1; j <= 1; ++j) {
            ...
            if (checkBoard(newX, newY) && (board[newX][newY].team != player)) {
...
bool getUserMove(const vector<pair<int, int>>& possibleMoves, pair<int, int>& selectedMove) {
    cout << "Possible moves: ";
...
bool step(int x, int y, int player) {
    ...
    switch (board[x][y].symbol) {
        ...
        case 'P':
            {
                vector<pair<int, int>> possibleMoves = allMoves_P(x, y, player);
                pair<int, int> selectedMove;

                while (!getUserMove(possibleMoves, selectedMove)) {
                }
        ...
        case 'R':
            {
                vector<pair<int, int>> possibleMoves = allMoves_R(x, y, player);
                pair<int, int> selectedMove;
        ...
        case 'N':
            {
                vector<pair<int, int>> possibleMoves = allMoves_N(x, y, player);
                pair<int, int> selectedMove;
        ...
        case 'B':
            {
                vector<pair<int, int>> possibleMoves = allMoves_B(x, y, player);
                pair<int, int> selectedMove;
        ...
        case 'Q':
            {
                vector<pair<int, int>> possibleMoves = allMoves_Q(x, y, player);
                pair<int, int> selectedMove;
        ...
        case 'K':
            {
                vector<pair<int, int>> possibleMoves = allMoves_K(x, y, player);
                pair<int, int> selectedMove;
